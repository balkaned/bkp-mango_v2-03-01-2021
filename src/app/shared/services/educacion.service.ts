...
export class EducacionService {
  ...
  crearEducacion( educacion: Educacion) {
    ...
    return this.http.post(`${ this.url }/educaciones`, educacion)
            .pipe(
              //Método "pipe" de los observables
              map( (resp:any) => {
                //El map recibe la respuesta de la petición, lo que responda http.post en este caso (Id)
                educacion.idEducacion = resp.idEducacion;
                return educacion; //Retorna el héroe ya manipulado (una instancia)
              })
            );
  ...
  getEducaciones(idpostulante: Number) {
    return this.http.get(`${ this.url }/educaciones/obtenerEducacionPorIdPostulante?idpostulante=${idpostulante}`)
            .pipe(
              map( this.crearArreglo ),
              delay(1500)
            );
  ...
  private crearArreglo( educacionObj: object ) {

    const educaciones: Educacion[] = [];

    //Hago un recorrido de obj y le asigno un "key"
    Object.keys( educacionObj ).forEach( key => {

      //Extraigo el objeto y lo asigno a una referencia
      const educacion: Educacion = educacionObj[key];
